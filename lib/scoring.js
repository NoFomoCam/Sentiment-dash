...
export function getZone(score) {
  if (score < 20) return { label: 'EXTREME BUY', color: '#23d18b' };
  if (score < 35) return { label: 'BUY / FEAR', color: '#5fd66e' };
  if (score < 45) return { label: 'WATCH', color: '#8fe04f' };
  if (score < 55) return { label: 'NEUTRAL', color: '#f7b737' };
  if (score < 65) return { label: 'CAUTION', color: '#fb8a3c' };
  if (score < 75) return { label: 'CAUTION / GREED', color: '#f6704a' };
  return { label: 'EXTREME SELL', color: '#f64f68' };
}
