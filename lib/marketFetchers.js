...
export async function refreshAll({ dry = false } = {}) {
  ...
  // Read market_data back (now including today's upsert), then persist scores
  // and generate the day's brief from the same snapshot.
  let scores, brief;
  try {
    ...
    scores = scores ?? { error: String(e) };
    brief = brief ?? { error: String(e) };
  }
